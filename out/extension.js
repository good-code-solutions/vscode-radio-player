...
const RadioPlayerViewProvider_1 = require("./RadioPlayerViewProvider");
let mainStatusBarItem;
let playPauseStatusBarItem;
let stationStatusBarItem;
function activate(context) {
    ...
    const provider = new RadioPlayerViewProvider_1.RadioPlayerViewProvider(context.extensionUri);
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(RadioPlayerViewProvider_1.RadioPlayerViewProvider.viewType, provider, {
    ...
    // Register Commands
    context.subscriptions.push(vscode.commands.registerCommand('vscode-radio-player.open', () => {
        showRadioMenu(context);
    }), vscode.commands.registerCommand('vscode-radio-player.play', () => {
        RadioPlayerViewProvider_1.RadioPlayerViewProvider.instance?.play();
    }), vscode.commands.registerCommand('vscode-radio-player.pause', () => {
        RadioPlayerViewProvider_1.RadioPlayerViewProvider.instance?.pause();
    }), vscode.commands.registerCommand('vscode-radio-player.pickStation', () => {
        pickStation(context);
    }));
    // Listen for status updates
    context.subscriptions.push(RadioPlayerViewProvider_1.RadioPlayerViewProvider.onStatusUpdate(status => {
        updateStatusBar(status);
    }));
    context.subscriptions.push(mainStatusBarItem, playPauseStatusBarItem, stationStatusBarItem);
}
async function showRadioMenu(context) {
    ...
    if (!RadioPlayerViewProvider_1.RadioPlayerViewProvider.instance) {
...
async function pickStation(context) {
    ...
    if (selected) {
        ...
        RadioPlayerViewProvider_1.RadioPlayerViewProvider.instance?.setStation(selected.station.name, selected.station.url);
    }
}
